...
def isolate_red_signs(image_path):
    ...
    # NEW: Apply a slight Gaussian Blur to smooth out pixelation and noise
...
def clean_mask_and_get_bounding_box(original_img_rgb, binary_mask):
    ...
    # EDITED: Reduced kernel to 3x3 so we don't crush tiny 32x32 pixel images
    ...
    if contours:
        ...
        # EDITED: Dropped area threshold from 500 to 50 to catch small GTSRB signs
...
def extract_features(cropped_rgb_img):
    """Uses ORB to find keypoints in the cropped image and draws them."""
    
    if cropped_rgb_img is None:
        print("No image to process!")
        return None, None

    # 1. Convert to Grayscale (ORB relies on intensity, so color isn't needed here)
    gray = cv2.cvtColor(cropped_rgb_img, cv2.COLOR_RGB2GRAY)

    # 2. Initialize the ORB detector
    # nfeatures controls the maximum number of points to find
    orb = cv2.ORB_create(nfeatures=500)

    # 3. Detect keypoints and compute descriptors
    # Keypoints are the (x,y) locations.
    # Descriptors are mathematical arrays describing the visual area around each keypoint.
    ...
    # cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS draws circles representing 
...
def match_features(cropped_rgb, kp1, des1, template_path):
    """Matches features from the cropped image against a perfect template."""
    ...
    # 2. Extract ORB features from the template
    orb = cv2.ORB_create(nfeatures=500)
    ...
    # 3. Initialize the Brute Force Matcher
    # NORM_HAMMING is the required distance metric for ORB
    # crossCheck=True ensures mutual matching (Point A matches Point B, and vice-versa)
    ...
    # 6. Draw the top matches (let's draw up to the top 15)
    # flags=2 (DRAW_MATCHES_FLAGS_NOT_DRAW_SINGLE_POINTS) hides unmatched points to keep it clean
    ...
    plt.title(f"Top {min(15, len(matches))} Feature Matches")
    plt.axis('off')
    plt.show()
...
if __name__ == "__main__":
    # Using your GTSRB test image path
    # sample_path = r"C:\Users\Lenovo\.cache\kagglehub\datasets\meowmeowmeowmeowmeow\gtsrb-german-traffic-sign\versions\1\Train\28\00028_00003_00005.png"
    # sample_path = r"C:\Users\Lenovo\.cache\kagglehub\datasets\meowmeowmeowmeowmeow\gtsrb-german-traffic-sign\versions\1\Train\28\00028_00002_00029.png"
